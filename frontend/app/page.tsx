...
export default function Home() {
  ...
  const [singleOrigin, setSingleOrigin] = useState<'input' | 'community'>('input')
  ...
  function handleSingleSampleLoaded(sample: SingleSampleData, origin: 'input' | 'community' = 'input') {
    setInputError('')
    ...
    setSingleOrigin(origin)
    setPageState('singleClaims')
    if (origin === 'community') setActiveTab('verify')
  }

  function handleSingleBack() {
    setPageState('input')
    if (singleOrigin === 'community') {
      setSingleOrigin('input')
  ...
  function renderVerifyContent(): ReactNode {
    ...
    if (pageState === 'singleClaims' && singleData) {
      return <SingleResultPage data={singleData} topic={topic} onBack={handleSingleBack} onVerifyClaim={handleVerifySingleClaim} />
    }
  ...
  return (
    <div className="min-h-[100dvh] bg-white">
      <div className="pb-16">
        {activeTab === 'verify' ? renderVerifyContent() : activeTab === 'community' ? <CommunityTab onOpenVerifiedCase={(sample) => handleSingleSampleLoaded(sample, 'community')} /> : <ProfileTab />}
      </div>
