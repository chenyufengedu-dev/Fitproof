...
export default function VerifyTopBar({ topic, page, total, onBack, hideProgress = false, rightAction }: VerifyTopBarProps) {
  return (
    <header className="relative z-10 flex shrink-0 items-center justify-between gap-2 px-5 py-2">
      <button type="button" onClick={onBack} className="inline-flex h-8 shrink-0 items-center rounded-full bg-white/75 px-3 text-sm font-medium text-[#128f80] shadow-sm backdrop-blur transition hover:bg-[#20CDB6] hover:text-white">‹ 返回</button>
      <div className="inline-flex h-8 min-w-0 max-w-[42%] items-center truncate rounded-full border border-[#20CDB6]/15 bg-white/75 px-3 text-center text-sm font-semibold text-[#128f80] shadow-sm backdrop-blur">
