...
export default function CourtCardShell({ label, index, subtitle, subtitleLeading, hideHeader = false, hideIndex = false, headerBadge, contentScrollable = true, children }: CourtCardShellProps) {
  return (
    <div className="flex h-full w-full flex-col overflow-hidden rounded-[24px] bg-white">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className={`mt-[2px] min-w-0 ${headerBadge && !hideIndex ? 'min-h-[74px]' : 'min-h-[46px]'}`}>
            <div className="flex min-w-0 items-center gap-1.5">
              <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-[#20CDB6]" />
            ...
            <div className={`flex shrink-0 flex-col items-end gap-1 ${hideIndex ? 'mt-[10px]' : ''}`}>
              {!hideIndex && <span className="text-[46px] font-black leading-none tracking-tight text-[#20CDB6]/25">
      ...
      <div className={`min-h-0 flex-1 ${contentScrollable ? 'overflow-y-auto' : 'overflow-hidden'} ${hideHeader ? 'px-5 pb-4 pt-5' : 'px-4 pb-2 pt-2'}`}>
        {children}
      </div>
