  ...
  return (
    <div className={`flex h-full w-full flex-col overflow-hidden bg-white ${dualShell ? 'rounded-[28px] border border-[#CDEDE7] shadow-[0_18px_52px_rgba(18,116,103,0.14)]' : 'rounded-[24px]'}`}>
        ...
        <div className="flex items-start justify-between gap-3">
          <div className={`min-w-0 ${subtitle ? (headerBadge && !hideIndex ? 'min-h-[74px]' : 'min-h-[46px]') : 'min-h-[23px]'}`}>
            ...
            <div className="min-w-0">
              {subtitle && <p className="mt-1.5 flex items-center gap-1 truncate text-[11px] leading-[14px] text-slate-600">{subtitleLeading}{subtitle}</p>}
            </div>
          </div>
      ...
      <div className={`${dualShell ? 'mx-5' : 'mx-4'} border-t border-[#20CDB6]/10 py-2`}>
        <p className="flex items-center justify-center gap-1.5 text-center text-[11px] leading-tight text-slate-600">
