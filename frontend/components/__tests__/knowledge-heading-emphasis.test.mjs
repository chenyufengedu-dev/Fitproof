...
test('knowledge heading and approved contribution count use the theme color', () => {
  assert.match(source, /data-knowledge-title[^>]*className="[^"]*text-\[#078C7E\]/s)
  assert.match(source, /data-approved-contribution-count[^>]*className="[^"]*t-body[^"]*font-black[^"]*text-\[#078C7E\]/s)
